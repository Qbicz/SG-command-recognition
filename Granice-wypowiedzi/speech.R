...
#nazwa pliku: 	speech.R
#nazwa funkcji:	speechBorder_wav()
#Wymagania:    	tuneR library
#			Pierwsze 100ms transmisji jest ciszą!!! 
#Argumenty wejściowe:
#filePath - ścieżka do pliku *.wav - gdy plik w folderze roboczym:'fileName.wav'
...
#drawPlot - rysjue wykresy wynikowych plików
#  0  -  nieaktywna
#  1  -  aktywna
...
speechBorder_wav <- function(filePath,mode,returnValue, drawPlot){

library(tuneR); #jeżeli wcześniej była załadowana, to można zakomentować

sndObj <- readWave(filePath);
y <- abs(sndObj@left);
doZapisu<-sndObj@left;
Fs <- sndObj@samp.rate;
